## This file contains masking head technique for ViT based on the LTH but by using attn * grad_attn. This is an
        ...
        for layer, layer_grad in zip(attentions, attention_grads):
            ...
            for batch, batch_grad in zip(layer, layer_grad):
                for idx, (head, head_grad) in enumerate(zip(batch, batch_grad)):
                    ## Attempt 1
                    # head_sensitivity[idx] += torch.matmul(head, head_grad).abs()[0, 1:].sum(dim=0).detach().cpu()
                    ## Attempt 2
                    # head_sensitivity[idx] += torch.matmul(torch.t(head), head_grad).abs()[0, 1:].sum(dim=0).detach().cpu()
                    ## Attempt 3
                    # head_sensitivity[idx] += torch.matmul(torch.t(head[0, 1:]).abs(), head_grad[0, 1:].abs()).detach().cpu()
                    ## Attempt 4
                    # head_sensitivity[idx] += torch.matmul(torch.t(head[0, 1:]), head_grad[0, 1:]).abs().detach().cpu()
                    ## Attempt 5
                    # head_sensitivity[idx] += torch.matmul(torch.clamp(head[0, 1:], min=0), torch.clamp(head_grad[0, 1:], min=0)).detach().cpu()
                    ## Attempt 6
                    # head_sensitivity[idx] += (head * head_grad).abs()[0, 1:].sum().detach().cpu()/(14 * 14)
                    ## Attempt 7
...
def reset_model(args):
    model = ViTForImageClassification.from_pretrained("starting_checkpoint_" + args.dataset_name + '_' + str(args.iteration_id) + "/checkpoint-100")
...
def main():
    ...
    training_args = TrainingArguments(output_dir="./starting_checkpoint_" + args.dataset_name + '_' + args.iteration_id,
        ...
        load_best_model_at_end=False, disable_tqdm=True, log_level='critical', )
    ...
    args.training_args = TrainingArguments(output_dir="./pruning_checkpoints_" + args.dataset_name + '_' + args.iteration_id,
                                           per_device_train_batch_size=16,
