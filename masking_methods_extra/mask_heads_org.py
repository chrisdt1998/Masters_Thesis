...
def compute_heads_importance(args, model, eval_dataloader, compute_entropy=True, compute_importance=True,
    ...
    for step, batch in enumerate(tqdm(eval_dataloader, desc="Iteration", disable=args.local_rank not in [-1, 0])):
        ...
        loss, logits, all_attentions = (
            outputs[0], outputs[1], outputs[-1],)  # Loss and logits are the first, attention the last
...
def mask_heads(args, model, eval_dataloader):
    ...
    while current_score >= original_score * args.masking_threshold:
        ...
        _, head_importance, preds, labels = compute_heads_importance(args, model, eval_dataloader,
            compute_entropy=False, head_mask=new_head_mask)
        preds = np.argmax(preds, axis=1) if args.output_mode == "classification" else np.squeeze(preds)
...
def prune_heads(args, model, eval_dataloader, head_mask):
    ...
    _, _, preds, labels = compute_heads_importance(args, model, eval_dataloader, compute_entropy=False,
        compute_importance=False, head_mask=head_mask)
    preds = np.argmax(preds, axis=1) if args.output_mode == "classification" else np.squeeze(preds)
    ...
    before_time = datetime.now()
    _, _, preds, labels = compute_heads_importance(args, model, eval_dataloader, compute_entropy=False,
        compute_importance=False, head_mask=None)
    preds = np.argmax(preds, axis=1) if args.output_mode == "classification" else np.squeeze(preds)
    score_pruning = compute_metrics(args.task_name, preds, labels)[args.metric_name]
    new_time = datetime.now() - before_time

    logger.info("Pruning: original num of params: %.2e, after pruning %.2e (%.1f percents)", original_num_params,
        pruned_num_params, pruned_num_params / original_num_params * 100, )
...
def main():
    ...
    parser.add_argument("--save_mask_all_iterations", action="store_true",
        help="Saves the masks and importance scores in all iterations")
    ...
    parser.add_argument("--use_train_data", action="store_true", help="Use training set for computing masks")
    ...
    if args.metric_name is None:
        args.metric_name = \
        {"cola": "mcc", "mnli": "acc", "mnli-mm": "acc", "mrpc": "acc", "sst-2": "acc", "sts-b": "corr", "qqp": "acc",
            "qnli": "acc", "rte": "acc", "wnli": "acc", }[args.task_name]
    ...
    config = config_class.from_pretrained(args.config_name if args.config_name else args.model_name_or_path,
        num_labels=num_labels, finetuning_task=args.task_name, output_attentions=True,
