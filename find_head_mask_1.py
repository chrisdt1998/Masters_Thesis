## This file contains masking head technique for ViT based on the LTH but using the gradients of the head mask. This is
# an iterative pruning approach.
import logging
import os
import argparse
...
def mask_heads(args):
    ...
    i = 0
    final_scores = []
    output_heatmap_mask = np.ones((12, 12)) * 13
...
def reset_model(args):
    model = ViTForImageClassification.from_pretrained("starting_checkpoint_" + args.dataset_name + '_' + str(args.iteration_number) + "/checkpoint-100")
...
def main():
    ...
    parser.add_argument('--iteration_number', type=int, default=0,
                        help='Iteration number to be able to run multiple scripts at the same time.')
    ...
    training_args = TrainingArguments(output_dir="./starting_checkpoint_" + args.dataset_name + '_' + str(args.iteration_number),
    ...
    args.training_args = TrainingArguments(output_dir="./pruning_checkpoints_" + args.dataset_name + '_' + str(args.iteration_number),
                                           per_device_train_batch_size=16,
